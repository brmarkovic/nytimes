...
export const XstateSimple17Machine = Machine<Icontext, Istates, Ievents>({
  id: 'XstateSimple17Machine',
  initial: 'ssr',
  // BIKA FOKUS >>>>>>>>>>
  context: {
    naziv: '',
      ...
      invoke: {
        src: async (cx, ev) => {
          const [ERRserverData, serverData] = await axios({
            ...
            .then((r) => [null, r])
            .catch((e) => [e]);
          if ((serverData && serverData.error) || ERRserverData) {
            throw new Error('error');
          }
          return serverData;
        },
          ...
          actions: [
            assign((cx, ev) => {
              cx.prognoza = {
                ...
                temp: ev?.data?.data?.current?.temp,
              };
            }),
          ],
